...
export const getUserNotifications = async (request, response) => {
  ...
  try {
    ...
    if (totalNotifications === 0) return response.status(200).json({ message: `You don't have any notifications` })
...
export const getUserMentions = async (request, response) => {
  ...
  try {
    ...
    if (totalNotifications === 0) return response.status(200).json({ message: `You don't have any mentions` })
...
export const toggleReadNotification = async (request, response) => {
  ...
  try {
    ...
    if (!notification) {
      return response.status(404).json({ error: `Notification ${notificationId} not found` })
    }
    ...
    return response.status(200).json({
      message: `Notification ${notificationId} marked as ${read ? 'read' : 'unread'}`
    })
  } catch (error) {
    console.log(error)
    return response.status(400).json({ error: `Error marking notification ${notificationId} as read` })
  }
}
...
export const markAllNotificationsAsRead = async (request, response) => {
  ...
  try {
    ...
    console.log(error)
    return response.status(400).json({ error: `Error marking all notifications as read` })
  }
}
...
export const deleteNotification = async (request, response) => {
  ...
  try {
    ...
    if (result.modifiedCount === 0) {
      console.error('No document was updated. Check if the conditions are correct.')
    ...
    console.log(error)
    return response.status(400).json({ error: `Error deleting notification ${notificationId}` })
  }
}
...
export const deleteAllNotifications = async (request, response) => {
  ...
  try {
    ...
    console.log(error)
    return response.status(400).json({ error: `Error deleting all notifications` })
  }
}
