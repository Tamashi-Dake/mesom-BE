...
export const getSetting = async (request, response) => {
  ...
  try {
    ...
    return response.status(400).json({ message: 'Error while fetching user setting' })
  }
}
...
export const updateSetting = async (request, response) => {
  ...
  try {
    ...
    return response.status(200).json({ message: 'Setting updated successfully' })
  } catch (error) {
    return response.status(400).json({ message: 'Error while updating user setting' })
  }
}
