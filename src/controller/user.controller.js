...
import { deleteUserById, getUserById, getUserByUsername, User } from '../db/user.model.js'
  ...
  try {
    ...
    if (files.avatarImg && files.avatarImg.length > 0) {
      if (user.profile.avatarImg) {
        await cloudinary.uploader.destroy(`Mesom/AvatarImage/${user.profile.avatarImg.split('/').pop().split('.')[0]}`)
      }
      const avatarResult = await streamUpload(files.avatarImg[0].buffer, 'Mesom/AvatarImage')
      user.profile.avatarImg = avatarResult.secure_url
    }

    if (files.coverImg && files.coverImg.length > 0) {
      if (user.profile.coverImg) {
        await cloudinary.uploader.destroy(`Mesom/CoverImage/${user.profile.coverImg.split('/').pop().split('.')[0]}`)
      }
      const coverResult = await streamUpload(files.coverImg[0].buffer, 'Mesom/CoverImage')
...
export const toggleFollowUser = async (request, response) => {
  ...
  try {
    ...
    const currentUserError = checkFollowLimit(currentUser.verified, 'following', currentUser.following.length)
    if (currentUserError) {
      return response.status(403).json(currentUserError)
    }

    // Check target user's followers limit
    const targetUserError = checkFollowLimit(targetUser.verified, 'followers', targetUser.followers.length)
    if (targetUserError) {
