...
export const checkCreateConversationConditions = async (request, response) => {
  ...
  try {
    ...
    if (!verified && participants.length > 4) {
      return response.status(400).json({
        message: 'You need to be verified to create a conversation with more than 5 participants'
      })
    }
    if (participants.length > 9) {
      return response.status(400).json({
        message: 'You can only create a conversation with a maximum of 10 participants'
      })
    }
    ...
    if (files.avatar && files.avatar.length > 0) {
      if (conversation.avatar) {
        await cloudinary.uploader.destroy(
          `Mesom/ConversationImage/${conversation.avatar.split('/').pop().split('.')[0]}`
        )
      }
      const avatarResult = await streamUpload(files.avatar[0].buffer, 'Mesom/ConversationImage')
