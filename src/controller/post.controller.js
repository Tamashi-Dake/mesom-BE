...
export const createPost = async (req, res) => {
  ...
  try {
    if (!text && files.length === 0) {
      return res.status(400).json({ error: 'Please provide text or image in the post' })
    }
...
export const getAllPosts = async (request, response) => {
  ...
  try {
    ...
    const posts = await Post.find({
      ...
      .populate({
        path: 'author',
        select: 'displayName username profile.avatarImg profile.coverImg profile.bio following followers'
      })
...
export const getPostsByFollowing = async (request, response) => {
  ...
  try {
    ...
    if (totalPosts === 0) return response.status(202).json({ message: 'No posts from people you follow yet.' })
    ...
    if (!posts) {
      return response.status(404).json({ message: "You aren't following anyone" })
    }
...
export const getPostsByUser = async (request, response) => {
  ...
  try {
    ...
    if (totalPosts === 0) return response.status(202).json({ message: "This user hasn't posted yet" })
...
export const getRepliesByUser = async (request, response) => {
  ...
  try {
    ...
    if (totalPosts === 0) return response.status(202).json({ message: "This user hasn't replied yet" })
...
export const getMediasByUser = async (request, response) => {
  ...
  try {
    ...
    if (totalPosts === 0) return response.status(202).json({ message: "This user hasn't posted any photos yet" })
...
export const getLikedPostsByUser = async (request, response) => {
  ...
  try {
    ...
    if (totalLikedPosts === 0) return response.status(202).json({ message: "This user hasn't liked any posts yet" })
    ...
    if (!posts) {
      return response.status(404).json({ message: `@${user.username} haven't liked any post` })
    }
...
export const getUserBookmarks = async (request, response) => {
  ...
  try {
    ...
    if (totalPosts === 0) {
      return response.status(200).json({ message: "You don't have any bookmarks" })
    }
    ...
    if (invalidPostIds.length > 0) {
      await User.updateOne({ _id: userId }, { $pull: { bookmarks: { post: { $in: invalidPostIds } } } })
    }

    const totalValidPosts = validBookmarks.length

    if (totalValidPosts === 0) {
      return response.status(200).json({ message: 'All your bookmarks have been deleted or invalid' })
    }

    // Sắp xếp lại các bookmarks hợp lệ theo thời gian bookmarked
    const sortedBookmarks = validBookmarks.sort((a, b) => b.bookmarkedAt - a.bookmarkedAt).slice(skip, skip + limit)
    ...
    // Bảo toàn thứ tự
    const sortedPosts = postIds.map((id) => posts.find((post) => post._id.toString() === id.toString()))
...
export const getRepliesForPost = async (request, response) => {
  ...
  try {
    ...
    if (totalReplies === 0) return response.status(202).json({ message: 'This post has no replies yet' })
    ...
    if (!isLiked) {
      ...
      if (post.author.toString() !== userID) {
        if (likeNotification) {
          ...
          await Notification.updateOne({ _id: likeNotification._id }, { show: true })
      ...
      if (likeNotification) {
        await Notification.updateOne({ _id: likeNotification._id }, { show: false })
      }
    }
    ...
    if (!isShared) {
      ...
      if (post.author.toString() !== userID) {
        if (shareNotification) {
          ...
          await Notification.updateOne({ _id: shareNotification._id }, { show: true })
        } else if (!request.blockedNotification) {
          // Tạo thông báo mới
          await Notification.create({
            from: userID,
      ...
      if (shareNotification) {
        await Notification.updateOne({ _id: shareNotification._id }, { show: false })
      }
    }
...
export const toggleBookmarkPost = async (request, response) => {
  ...
  try {
    ...
    const isBookmarked = user.bookmarks.some((bookmark) => bookmark.post.toString() === postID.toString())
    ...
    if (!isBookmarked) {
      ...
      user.bookmarks = user.bookmarks.filter((bookmark) => bookmark.post.toString() !== postID.toString())
    }
